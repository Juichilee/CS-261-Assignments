...
void _swap(DynArr *da, int i, int j);
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
	...
	if(l < max && heap->data[l].priority < heap->data[largest].priority){
		largest = l;
	}
	
	if(r < max && heap->data[r].priority < heap->data[largest].priority){
		largest = r;
	}
		
	if(largest != pos){
		_swap(heap, pos, largest);
